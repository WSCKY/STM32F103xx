...
#elif (TEST_OPTION == 1)
static uint8_t tx_msg1[20] = "01234567890123456789";
static uint8_t tx_msg2[40] = "01234567890123456789""01234567890123456789";
static uint8_t tx_msg3[60] = "01234567890123456789""01234567890123456789""01234567890123456789";
#else

#endif

#if (TEST_OPTION == 1)
void meas_TX_time(void)
{
  uint32_t sTime = 0, eTime = 0;

  dwt_writetxdata(20, tx_msg1, 0); /* Zero offset in TX buffer. */
  dwt_writetxfctrl(20, 0, 1); /* Zero offset in TX buffer, ranging. */
  MonitorUpdateDataPos(200, 0);
...
sTime = _Get_Micros();
 ...
 eTime = _Get_Micros();
  ...
  dwt_writetxdata(40, tx_msg2, 0); /* Zero offset in TX buffer. */
  dwt_writetxfctrl(40, 0, 1); /* Zero offset in TX buffer, ranging. */
  MonitorUpdateDataPos(250, 0);
...
sTime = _Get_Micros();
 ...
 eTime = _Get_Micros();
  ...
  dwt_writetxdata(60, tx_msg3, 0); /* Zero offset in TX buffer. */
  dwt_writetxfctrl(60, 0, 1); /* Zero offset in TX buffer, ranging. */
  MonitorUpdateDataPos(300, 0);
